...
% Both Spring WebFlux and Vert.X web frameworks provide native support for Kotlin
% \textit{coroutines}, enabling developers to utilize a specific
% \texttt{Dispatcher} to switch the execution thread context to a suitable
% subsystem and reduce \textit{system-level} blocking.
...
% We will demonstrate this capability by comparing it with our proposal of
% \textit{suspendable web templates} in a benchmark using Spring WebFlux.

% Determining the appropriate size of \emph{N} for the I/O susbsystem may hinge on
% factors such as the nature of the workload and the frequency of
% \textit{Blocking} I/O operations.
