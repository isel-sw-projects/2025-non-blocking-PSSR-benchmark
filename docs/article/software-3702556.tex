...
\Author{\hl{Bernardo Pereira} %MDPI: 1. Please carefully check the accuracy of names and affiliations. 
 \orcidA{} and \hl{Fernando Miguel Carvalho} %MDPI: The names highlighted are different from the ones submitted online at susy.mdpi.com. Please confirm which are correct. 
 *\orcidB{}}
...
% Lastname, F.; Lastname, F.; Lastname, F.
 \isAPAStyle{%
        \AuthorCitation{Lastname, F., Lastname, F., \& Lastname, F.}
         ...
         \AuthorCitation{\hl{Pereira}%MDPI: Please carefully check the accuracy of names
, B.; Carvalho, F.M.}
         }
 }

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address[1]{Polytechnical Institute of Lisbon, \hl{ISEL}%MDPI: Please add full name if possible
, \hl{Lisbon}%MDPI: please add post code
, Portugal; \hl{a50493@alunos.isel.pt} %MDPI: We added these email addresses here according to those submitted online at susy.mdpi.com. Please confirm.
}
...
performance, user experience, and \textit{\hl{scalability}%MDPI: Please confirm if the italics are necessary; if not, please remove them. The following highlights are the same.
...
can increase server load and reduce \textit{\hl{throughput}} ({\hl{The} %MDPI: footnote is not allowed in this journal, so we moved it to here, please confirm it
 number of
...
With the introduction of \textit{\hl{Virtual
    Threads}}~({\url{https://openjdk.org/jeps/444}}, \hl{accessed on}%MDPI: footnote is not allowed in this journal, so we moved it to here, please confirm it, and please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
...
\textit{\hl{nested function}} idiom used by \textbf{\hl{ScalaTags}%MDPI: Please confirm if the bold formatting is necessary; if not, please remove it. The following highlights are the same.
}, as shown in the
...
languages like \hl{C\#, JavaScript, Python, Perl, Swift, Kotlin} %MDPI: Please confirm whether this is the software used in this article. If so, please provide the version number.
, and others,
...
include HtmlFlow using suspendable web templates (\textit{\hl{HtmlFlow-Susp}},
...
(\textit{\hl{Jstachio-Virtual}}), and Thymeleaf using the reactive View Resolver
driver (Thymeleaf-Rx). \textit{\hl{Blocking}} and \textit{\hl{Virtual}} represent the
...
\textit{\hl{HtmlFlow-Susp}} scales up to 128 concurrent users, achieving \hl{4487} %MDPI: Commas are only used for numbers with five or more digits. We have removed them in four-digit numbers. Please confirm.
...
\section{\highlighting{Discussion} %MDPI: To AE: This section seems to be added compared to the PDF file from susy, please check if it is correct.
}\label{s6}
...
\dataavailability{The data presented in this study are available in Github at \url{https://github.com/xmlet/comparing-non-blocking-progressive-ssr}, \hl{accessed on}.}
...
\newblock In \emph{Web Information Systems Engineering---WISE
  2024}; Barhamgi, M., Wang, H., Wang, X., Eds.; \hl{Springer}%MDPI: newly added, please confirm it
...
\newblock Technical Report, 2017. Available online: \url{https://htmlflow.org/} \hl{(accessed on).} %MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
...
\hl{Fowler, M.} %MDPI: Refs.7 and 15 are duplicated. Please remove duplicated references and rearrange all the references so that they appear in numerical order. Please ensure that there are no duplicated references.
\newblock {\em Domain Specific Languages}; Addison-Wesley Professional: \hl{Boston, MA, USA,} 2010.
...
\newblock {\em Domain-Driven Design: Tackling Complexity in the Heart of
  Software}; Addison-Wesley: \hl{Boston, MA, USA,} 2004.

\bibitem[Fowler(2010)]{dslbook}
\hl{Fowler, M.}
\newblock {\em Domain Specific Languages}, 1st ed.; Addison-Wesley
...
Resig, J.
\newblock {\em Pro JavaScript Techniques}; Apress: \hl{New York, NY, USA,} 2007.
...
\newblock In Proceedings of the Internaional Softare Development Conference, \hl{Vancouver, BC, Canada, 16--24 May 2009} %MDPI: 
;  Available online: \url{https://qconsf.com/sf2009/sf2009/speaker/Erik+Meijer.html} \hl{(accessed on).} %MDPI: 
...
\newblock User Acceptance of Modified Web Page Loading Based on Progressive
  Streaming.
\newblock In Proceedings of the International Conference on Web \hl{Engineering, 2022;} %MDPI: please add location and date for conference
...
\newblock In Proceedings of the 48th ICT and Electronics Convention MIPRO 2025. Rijeka: Croatian Society for Information, Communication and Electronic Technology–MIPRO, \hl{Opatija, Croatia, 2--6 June 2025.} %MDPI: Please confirm added conference date and location.
